...
for file in *.mp3; do if [ -f "$file" ]; then LENGTH="$(ffprobe -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 "$file" 2>/dev/null)"; QUALITY="$(ffprobe -show_entries format=bit_rate -of default=noprint_wrappers=1:nokey=1 "$file" 2>/dev/null)"; SIZE="$(stat --format=%s "$file" 2>/dev/null)";  SUM="$(sha256sum "$file" | cut -d " " -f 1 )"; echo $SUM\\t$file\\t$LENGTH\\t$QUALITY\\t$SIZE; fi; done | tee durations.txt
```
