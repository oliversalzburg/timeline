...
export const dateToString = (date: Date) =>
	`${date.getFullYear().toFixed().padStart(4, "0")}-${(date.getMonth() + 1).toFixed().padStart(2, "0")}-${date.getDate().toFixed().padStart(2, "0")}`;
export const dateToStringWithTime = (date: Date) =>
	`${dateToString(date)}T${date.getHours().toFixed().padStart(2, "0")}:${date.getMinutes().toFixed().padStart(2, "0")}:${date.getSeconds().toFixed().padStart(2, "0")}Z`;
...
export const serialize = (
	...
	const document: TimelineDocumentInternal = {
		...
		timeline: timeline.records.reduce((serialized, [timestamp, entry]) => {
			...
			const timeString = dateToStringWithTime(date);
